...
/* Register Application Analytics Here */
...
/* Total Revenue*/
$query = "SELECT p.product_retail_price*s.sale_quantity AS total_revenue FROM  sales s INNER JOIN products p ON p.product_id = s.sale_product_id";
$stmt = $mysqli->prepare($query);
$stmt->execute();
...
/* Store 1 Revenue  */
$query = "SELECT p.product_retail_price*s.sale_quantity
AS total_revenue FROM 
sales s INNER JOIN products p ON p.product_id = s.sale_product_id
WHERE p.product_access_level = 'store_1'";
$stmt = $mysqli->prepare($query);
$stmt->execute();
$stmt->bind_result($store_1_sales);
$stmt->fetch();
$stmt->close();
...
WHERE p.product_access_level = 'store_2';
";
$stmt = $mysqli->prepare($query);
$stmt->execute();
$stmt->bind_result($store_2_sales);
$stmt->fetch();
$stmt->close();
